...
function Banner() {
  return (
    <div className="relative">
      <img className="w-full xl:h-[580px] lg:h-[580px] md:h-[530px] h-[700px] object-cover" src={banner} alt="Banner" />
      <div className="absolute text-center xl:top-20 lg:top-20 md:top-20 top-24 left-0 right-0 bottom-0 flex flex-col items-center justify-center p-4">
        <div className="bg-white rounded-xl mb-2 xl:w-[90%] lg:w-[90%] md:w-[95%] w-full xl:px-14 lg:px-14 md:px-5 px-6 py-14 ">
          ...
          <div className="xl:flex lg:flex md:flex">
            <div className="grid xl:grid-cols-4 lg:grid-cols-4 md:grid-cols-4 grid-cols-1 gap-5 w-full">
              {infos.map((info, indx) => (
                <div key={indx}>
                  <div className="flex border-[1px] border-gray-200 items-center rounded-lg hover:bg-gray-100">
                    <h1 className="text-xl xl:m-4 lg:m-4 md:m-2 m-4 border-r-[2px] xl:pr-4 lg:pr-4 md:pr-2 pr-4 font-semibold">{info.heading}</h1>
                    <div>
                      <p className="text-base text-start font-medium">{info.title}</p>
                      <p className="lg:text-xs md:text-[10px] text-xs text-start">{info.text}</p>
                    </div>
                  </div>
                </div>
              ))}
            </div>
            <button className="ml-5 bg-yellow-500 rounded-xl w-28 hover:bg-yellow-600">
